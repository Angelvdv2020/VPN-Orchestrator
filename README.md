...
к edge/front, а также ключи REALITY/Hysteria2. Node Secret генерируются автоматически
(вручную запускать `openssl rand -hex 32` не нужно). Затем мастер установит компоненты,
подготовит обе ноды, соберёт подписку и сохранит bundle. При сетевой или установочной
ошибке он покажет причину и предложит повторить этап после исправления. Вводимые значения не
