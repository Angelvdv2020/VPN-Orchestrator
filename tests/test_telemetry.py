...
def test_store_summary_and_replay(tmp_path):
    ...
    payload = {"observed_at": 100, "path_id": "reality-xhttp", "status": "up",
               "network": "mobile", "latency_ms": 42}
...
def test_api_auth_replay_and_admin(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv(device_env_name(cfg.web.telemetry_key_prefix, "phone-1"), "device-secret")
    monkeypatch.setenv(cfg.web.admin_token_env, "admin-secret")
    client = TestClient(create_app(cfg))
    now = str(int(time.time()))
    nonce = "abcdefghijklmnop"
    payload = {"observed_at": int(now), "path_id": "reality-xhttp", "status": "up",
               "network": "mobile", "operator": "test", "country": "RU", "latency_ms": 55}
    body = json.dumps(payload, separators=(",", ":")).encode()
    headers = {"X-Device-ID": "phone-1", "X-Timestamp": now, "X-Nonce": nonce,
    ...
    assert client.post("/api/v1/telemetry", content=body, headers=headers).status_code == 409
    assert client.get("/api/v1/status").status_code == 401
    assert client.get("/api/v1/status", headers={"Authorization": "Bearer admin-secret"}).status_code == 200
    assert client.post(
        "/api/v1/telemetry",
        content=b"x",
