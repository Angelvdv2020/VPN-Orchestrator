...
def test_existing_env_secrets_are_not_rotated(tmp_path, monkeypatch):
    ...
    panel.install_panel(cfg, FakeRunner(), panel_domain="noryx.ru", subscription_domain="sub.noryx.ru", start=False)
    current = (tmp_path / ".env").read_text()
    for value in ("keep", "keep-auth", "keep-api", "keep-metrics", "keep-hook", "keep-db"):
