...
def device_env_name(prefix: str, device_id: str) -> str:
    ...
    normalized = re.sub(r"[^A-Za-z0-9]", "_", device_id).upper()
...
def sign(secret: str, timestamp: str, nonce: str, body: bytes) -> str:
    return hmac.new(secret.encode(), signing_message(timestamp, nonce, body), hashlib.sha256).hexdigest()


def verify(
    secret: str, timestamp: str, nonce: str, body: bytes, signature: str,
    *, max_skew: int, now: int | None = None,
) -> None:
